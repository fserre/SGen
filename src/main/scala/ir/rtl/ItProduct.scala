...
case class ItProduct[U](r: Int, factor: StreamingModule[U], endLoopOpt: Option[StreamingModule[U]] = None) extends StreamingModule[U](factor.t, factor.k)(using factor.hw) {
  ...
  override def implement(rst: Component, token: Int => Component, inputs: Seq[Component]): Seq[Component] = {
    ...
    val feeding2 = Mux(token(T - 1), Seq(feeding1, Const(1, 0)))
    val feeding3 = Mux(token(-1), Seq(feeding2, Const(1, 1)))
    val feeding = feeding3.register
    feeding1.input = feeding
    val loopOutputsCopy = Seq.fill(K)(new Wire(hw.size))
    val loopInputs = loopOutputsCopy.zip(inputs).map { case (i1, i2) => Mux(feeding, Seq(i1, i2)).register }
    val loopOutputs = factor.implement(rst, time => Or((0 until r).map(_ * innerLatency + time + 1).map(token)), loopInputs)
    val loopEndOutputs = endLoop.implement(rst, time => Or((0 until r - 1).map(_ * innerLatency + 1 + factor.latency + time).map(token)), loopOutputs)
