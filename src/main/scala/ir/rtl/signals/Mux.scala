...
case class Mux[U] private(address: Sig[Int], inputs: Seq[Sig[U]]) extends Operator[U](address +: inputs*)(using inputs.head.hw):
...
object Mux:
  def apply[U](address: Sig[Int], inputs: Seq[Sig[U]]): Sig[U] = 
    ...
    address match {
      case value: Const[Int] => inputs(value.value)
      case _ => if inputs.forall(_ == head) then
        head
          ...
          case _ => hw match
            ...
            case Unsigned(_) => (0 until hw.size).find(i => // search for a bit that is always the same across values
              ...
              case _ =>
                ...
                else if address.hw == hw && inputs.zipWithIndex.forall(i => i._1 == Not(Const(i._2)(using address.hw))) then
...
object ROM:
  ...
  def unapply[U](arg:Sig[U]):Option[(Seq[U],Sig[Int])]=arg match 
    case arg: Mux[U] if arg.isRom => Some(arg.inputs.map{case Const(value) => value },arg.address)
    case _ => None
