...
/**
 * Floating point representation that uses operators generated by Flopoco:
...
case class Flopoco(wE: Int, wF: Int) extends HW[Double](wE + wF + 3):
  ...
  lazy private val (latPlus, latDiff, latMult) = 
    ...
    if !java.nio.file.Files.isRegularFile(path) then
      ...
      println()
      println(s"flopoco outputFile=${path.toAbsolutePath} target=Virtex6 frequency=700 FPMult name=mult wE=$wE wF=$wF FPAdd name=add wE=$wE wF=$wF sub=false FPAdd name=diff wE=$wE wF=$wF sub=true")
      throw new Exception(s"Flopoco file not found for wE=$wE and wF=$wF")
    val source = Source.fromFile(filename)
    val flopoco = source.getLines().mkString
    source.close()
    var pos = flopoco.indexOf(s"FPAdd_${wE}_${wF}")
    pos = flopoco.indexOf(" Pipeline depth: ", pos) + 17
    var posEnd = flopoco.indexOf(" ", pos)
    val latPlus = flopoco.slice(pos, posEnd).toInt
    pos = flopoco.indexOf(s"FPSub_${wE}_${wF}")
    pos = flopoco.indexOf(" Pipeline depth: ", pos) + 17
    posEnd = flopoco.indexOf(" ", pos)
    val latDiff = flopoco.slice(pos, posEnd).toInt
    pos = flopoco.indexOf(s"FPMult_${wE}_${wF}")
    pos = flopoco.indexOf(" Pipeline depth: ", pos) + 17
    posEnd = flopoco.indexOf(" ", pos)
  ...
  override def valueOf(const: BigInt): Double =
    ...
    else if flags==5 then
      Double.NegativeInfinity
    else if flags==6 || flags==7 then
      Double.NaN
    else
