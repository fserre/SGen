...
case class ComplexHW[T](hw: HW[T]) extends HW[Complex[T]](hw.size * 2) (using Complex.complexIsFractional[T](using hw.num)):
  ...
  override def MID_VALUE: Complex[T] = Complex(hw.MID_VALUE)(using hw.num)

  override def MAX_VALUE: Complex[T] = ???

  override def values: Iterator[Complex[T]] =
    for 
      re <- hw.values
      im <- hw.values
    yield Complex(re, im)(using hw.num)
