...
case class TemporalSPRAM[U: HW] private(override val P3: Seq[Matrix[F2]], override val P4: Seq[Matrix[F2]]) extends SLP(P3.head.m, P3.head.n, P3.size) {
  // Only supports "strict" full-throughput
...
object TemporalSPRAM {
  def apply[U: HW](P3: Seq[Matrix[F2]], P4: Seq[Matrix[F2]]): SB[U] = if (P3.forall(_.isZero) && P4.forall(_.isIdentity))
    ...
    new TemporalSPRAM(P3, P4)

  def apply[U: HW](P3: Matrix[F2], P4: Matrix[F2]): SB[U] = TemporalSPRAM(Seq(P3), Seq(P4))
}