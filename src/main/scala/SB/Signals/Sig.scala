...
abstract class Sig[T](val parents: SigRef[_]*)(implicit val hw: HW[T],implicit val sb:SB[_]=parents.head.sb) {that=>
  ...
  def toString(s:SigRef[_] => String)=that.getClass.getSimpleName+parents.map(s).mkString("(",", ",")")
...
abstract class AssociativeSig[T](val terms:Seq[SigRef[T]],op:String,override val precedence:Int)(implicit hw:HW[T]=terms.head.hw) extends Sig(terms: _*)(hw) with AssociativeNode[Sig[T]]{that=>
