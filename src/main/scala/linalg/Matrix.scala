...
//    num.times(abs, abs)
   ...
   */
  lazy val reducedRowEchelonForm:Matrix[T] = reducedRowEchelonFormInt._1
  /**
   * Provides the reduced Column Echelon Form of this matrix
   */
  lazy val reducedColEchelonForm :Matrix[T]= reducedColEchelonFormInt._1
  lazy val isFullRank:Boolean = rk == Math.min(m, n)
  /**
  ...
  def isReverseIdentity:Boolean =  m == n && (0 until m).forall(j => (0 until m).forall(i => values(j*m+i)==(if (i == n-j-1)  num.one else  num.zero)))
   ...
   */
  def swapRows(row1: Int, row2: Int):Matrix[T] = Matrix.tabulate(m, n)((i, j) => if (i == row1) this (row2, j) else if (i == row2) this (row1, j) else this (i, j))
   ...
   */
  def swapCols(col1: Int, col2: Int):Matrix[T] = Matrix.tabulate(m, n)((i, j) => if (j == col1) this (i, col2) else if (j == col2) this (i, col1) else this (i, j))
   ...
   */
  def addRowTo(row1: Int, l: T, row2: Int):Matrix[T] = Matrix.tabulate(m, n)((i, j) => if (i == row2) num.plus(this (i, j), num.times(l, this (row1, j))) else this (i, j))
   ...
   */
  def addColTo(col1: Int, l: T, col2: Int):Matrix[T] = Matrix.tabulate(m, n)((i, j) => if (j == col2) num.plus(this (i, j), num.times(l, this (i, col1))) else this (i, j))
   ...
   */
  def mulRow(row: Int, l: T):Matrix[T] = {
    assert(l != num.zero)
   ...
   */
  def mulCol(col: Int, l: T):Matrix[T] = {
    assert(l != num.zero)
   ...
   */
  def cols:IndexedSeq[Vec[T]] = (0 until n).map(col)
...
object Matrix {
   ...
   */
  def fromVecs[T: Fractional](m: Int, values: Seq[Vec[T]]):Matrix[T] = {
