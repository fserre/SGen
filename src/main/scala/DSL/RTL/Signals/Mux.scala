...
import DSL.RTL.HardwareType.{ComplexHW, HW, Unsigned}
...
case class Mux[U] private(address: SigRef[Int], inputs: Seq[SigRef[U]]) extends Operator[U](address +: inputs: _*)(using inputs.head.hw) {
  ...
  override def implement(implicit cp: SigRef[?] => Component): Component = new DSL.RTL.Mux(cp(address), inputs.map(cp))
