...
final class And private (override val terms: Seq[SigRef[Int]]) extends AssociativeSig[Int](terms," & ",8){
  override def implement(implicit cp: SigRef[?] => Component): Component = new DSL.RTL.And(terms.map(cp))
...
case class Not private(input: SigRef[Int]) extends Operator[Int](input)(input.hw) {
  override def implement(implicit cp: SigRef[?] => Component): Component =new DSL.RTL.Not(cp(input))
}
...
final class Xor private(override val terms: Seq[SigRef[Int]]) extends AssociativeSig[Int](terms, " ^ ",9) {
  override def implement(implicit cp: SigRef[?] => Component): Component = new DSL.RTL.Xor(terms.map(cp))
  override val pipeline = 1
...
final class Concat private(override val terms: Seq[SigRef[Int]]) extends AssociativeSig[Int](terms," :: ",12)(Unsigned(terms.map(_.hw.size).sum)) {
  override def implement(implicit cp: SigRef[?] => Component): Component = new DSL.RTL.Concat(terms.map(cp))
...
case class Tap private(input: SigRef[Int], range: Range) extends Operator[Int](input)(Unsigned(range.size)) {
  override def implement(implicit cp: SigRef[?] => Component): Component = new DSL.RTL.Tap(cp(input), range)
}
