...
case class Complex[+T: Numeric](re: T, im: T):
   ...
   */
  override def toString: String = s"$re${if (im != implicitly[Numeric[T]].zero) s" + ${im}i" else ""}"
...
object Complex :
  ...
  given complexIsFractional[T](using num: Numeric[T]): Fractional[Complex[T]] with
    ...
    override def parseString(str: String): Option[Complex[T]] =
      ...
      val parts = cleanStr.split("(?=[+-])").filter(_.nonEmpty)
      parts.foldLeft[Option[Complex[T]]](Some(Complex(num.zero, num.zero)))((res, part) =>
        if part.endsWith("i") || part.endsWith("I") then
          val imagStr = part.dropRight(1)
          imagStr match
            case "" | "+" => res.map(_.copy(im=num.one))
            case "-" => res.map(_.copy(im = -num.one))
            case _ =>
              for
                complex <- res
                v <- num.parseString(imagStr)
              yield complex.copy(im = v)
        else
          for
            complex <- res
            v <- num.parseString(part)
          yield complex.copy(re = v)
      )
