...
object DFT {
  ...
  def CTDFT(n: Int, r: Int,dualPorted:Boolean): SPL[Complex[Double]] = {
    assert(n % r == 0)
    if (n == 1)
      DFT2()
    else
      LinearPerm(LinearPerm.Lmat(r, n),dualPorted) * Product(n / r)(l => ITensor(n - r, CTDFT(r, 1,dualPorted)) * DiagE(n, r, l) * Q(n, r, l,dualPorted)) * LinearPerm(LinearPerm.Rmat(r, n),dualPorted)
  }
  ...
  def Pease(n: Int, r: Int,dualPorted:Boolean): SPL[Complex[Double]] = {
    assert(n%r==0)
    if(n==1)
      DFT2()
    else
      LinearPerm(LinearPerm.Rmat(r, n),dualPorted) * Product(n / r)(l => DiagC(n, r,n/r-l-1) * ITensor(n-r,CTDFT(r, 1,dualPorted)) * LinearPerm(LinearPerm.Lmat(r, n).inverse,dualPorted))
  }
